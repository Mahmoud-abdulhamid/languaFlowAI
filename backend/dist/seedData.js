...
const seedData = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const role of DEFAULT_ROLES) {
            const exists = yield Role_1.Role.findOne({ name: role.name });
            if (!exists) {
                yield Role_1.Role.create(role);
            }
            else {
        ...
        for (const lang of languages) {
            const exists = yield Language_1.Language.findOne({ code: lang.code });
            if (!exists) {
                yield Language_1.Language.create(lang);
            }
        }
        // 3. Seed Users
        ...
        for (const u of demoUsers) {
            const existingUser = yield User_1.User.findOne({ email: u.email });
            if (!existingUser) {
                const salt = yield bcryptjs_1.default.genSalt(10);
                const hashedPassword = yield bcryptjs_1.default.hash(u.password, salt);
        ...
        yield Project_1.Project.deleteMany({}); // Clear existing projects
        console.log('Cleared existing projects.');
        // Fetch users to link
        const client = yield User_1.User.findOne({ email: 'client@example.com' });
        const mike = yield User_1.User.findOne({ email: 'mike@example.com' });
        const translatorAr = yield User_1.User.findOne({ email: 'translator@example.com' });
        const translatorEs = yield User_1.User.findOne({ email: 'sarah@example.com' });
        ...
        const projects = [
            ...
            },
            {
                ...
                status: 'ACTIVE',
                clientId: client._id,
                assignedTranslators: [translatorEs === null || translatorEs === void 0 ? void 0 : translatorEs._id],
                budget: 1200,
            ...
            },
            {
                ...
                targetLangs: ['fr', 'de'],
                status: 'DRAFT',
                clientId: (mike === null || mike === void 0 ? void 0 : mike._id) || client._id,
                budget: 3000,
        ...
        for (const p of projects) {
            yield Project_1.Project.create(p);
        }
